...
export default function MovieDetail() {
  const { addToFavorite, removeFromFavorite, isFavoriteMovie } =
    useFavoriteMovie();
  const { addToSavedMovie, removFromSavedMovie, isSavedMovie } =
    useSavedMovies();
  const params = useParams<{ movieId: string | string[] }>();
  const movieId = Array.isArray(params?.movieId)
    ? params.movieId[0]
    : params?.movieId;
  const [movieDetail, setMovieDetail] = useState<Awaited<
    ReturnType<typeof movieDetails>
  > | null>(null);
    ...
    <div className="relative dark:border-white h-full w-full flex p-2 sm:p-5 md:p-10 items-center overflow-auto shadow-4xl ">
      ...
      {toggleTrailer && (
        <div className="absolute top-0 h-full w-full left-0 z-100 p-10 bg-black/80 ">
          <IoCloseCircle
            className="absolute top-6 right-6 text-2xl z-100 text-white"
      ...
      <div className="flex flex-col gap-4 text-white dark:text-white max-w-150">
        ...
        <span className="flex gap-4">
          ...
          <button
            className="flex items-center gap-2 py-3 px-6 bg-white/10 backdrop-blur-md rounded-lg text-white "
            onClick={handleToggleTrailer}
          >
            {" "}
        ...
        <div className="flex gap-2 flex-wrap flex-col">
          <h1 className="drop-shadow-[4px_4px_10px_rgba(0,0,0,0.9)]">
            Top Casts:
          </h1>
          ...
          <span className="flex gap-2">
            ...
            <span className="flex flex-wrap gap-2 drop-shadow-[4px_4px_10px_rgba(0,0,0,0.9)]">
              {movieDetail.genres?.map((genre: any) => (
                <p
                  key={genre.id}
                  className="bg-white/10 backdrop-blur-md rounded-lg p-2 drop-shadow-[4px_4px_10px_rgba(0,0,0,0.9)]"
                >
                  {" "}
                  {genre.name}
                </p>
